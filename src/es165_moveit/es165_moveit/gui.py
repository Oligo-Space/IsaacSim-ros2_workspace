...
class MainWindow(QMainWindow):
    ...
    def make_gui(self):
        ...
        for i in range(3):
            ...
            reset_button = QPushButton("Reset")
            reset_button.setFixedWidth(50)
            reset_button.clicked.connect(lambda x: self.reset_boxes(torque_state_boxes))
            arm_box.addWidget(reset_button)
            box.addLayout(arm_box)
        control_tab_layout.addLayout(box)
        ################################################
        ...
        for i in range(3):
            ...
            rw_button.clicked.connect(lambda y, boxes=rw_state_boxes: self.publish_float_array(boxes,self.ros_node.set_rw_speed))
            arm_box.addWidget(rw_button)
            reset_button = QPushButton("Reset")
            reset_button.setFixedWidth(50)
            reset_button.clicked.connect(lambda x: self.reset_boxes(rw_state_boxes))
            arm_box.addWidget(reset_button)
            box.addLayout(arm_box)
        control_tab_layout.addLayout(box)
        #################################################
