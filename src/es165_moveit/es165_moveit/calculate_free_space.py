...
"""Generated by Claude Opus 4.5, verified in production"""
    ...
    def generate_poses_batch(self, batch_size: int = 1000):
        ...
        for x, y, z in product(self.x_range, self.y_range, self.z_range):
            ...
            for roll, pitch, yaw in product(self.roll_range, self.pitch_range, self.yaw_range):
                ...
                if not self.check_ground_plane(x, y, z, roll, pitch, yaw):
                    continue
...
if __name__ == "__main__":
    ...
    robot_file = "/home/oligo/IsaacSim-ros_workspaces/humble_ws/src/es165_moveit/urdf/robot_description.yaml"
    ...
    # orientation_steps = {
    #     'roll': np.deg2rad(180),   # 45° steps
    #     'pitch': np.deg2rad(180),
    #     'yaw': np.deg2rad(180),    # 90° steps
    # }

    # IK solver batch size (process this many poses at once)
    ik_batch_size = 2000  # Larger batch for better GPU utilization

    
    # ground_plane = Cuboid(
    #     name="ground",
    #     pose=[0.0, 0.0, -5.0, 0.0, 0.0, 0.0, 1.0],
    #     dims=[50.0, 50.0, 10.0],
    # )
