...
vision_msgs__msg__Detection2DArray__create(void)
{
  ...
  vision_msgs__msg__Detection2DArray__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    vision_msgs__msg__Detection2DArray * data =
      (vision_msgs__msg__Detection2DArray *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vision_msgs__msg__Detection2DArray__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          vision_msgs__msg__Detection2DArray__fini(&output->data[i]);
        }
        return false;
      }
    }
