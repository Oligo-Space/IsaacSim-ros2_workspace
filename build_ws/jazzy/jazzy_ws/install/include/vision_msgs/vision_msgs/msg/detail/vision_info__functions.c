// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from vision_msgs:msg/VisionInfo.idl
// generated code does not contain a copyright notice
#include "vision_msgs/msg/detail/vision_info__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `header`
#include "std_msgs/msg/detail/header__functions.h"
// Member `method`
// Member `database_location`
#include "rosidl_runtime_c/string_functions.h"

bool
vision_msgs__msg__VisionInfo__init(vision_msgs__msg__VisionInfo * msg)
{
  if (!msg) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__init(&msg->header)) {
    vision_msgs__msg__VisionInfo__fini(msg);
    return false;
  }
  // method
  if (!rosidl_runtime_c__String__init(&msg->method)) {
    vision_msgs__msg__VisionInfo__fini(msg);
    return false;
  }
  // database_location
  if (!rosidl_runtime_c__String__init(&msg->database_location)) {
    vision_msgs__msg__VisionInfo__fini(msg);
    return false;
  }
  // database_version
  return true;
}

void
vision_msgs__msg__VisionInfo__fini(vision_msgs__msg__VisionInfo * msg)
{
  if (!msg) {
    return;
  }
  // header
  std_msgs__msg__Header__fini(&msg->header);
  // method
  rosidl_runtime_c__String__fini(&msg->method);
  // database_location
  rosidl_runtime_c__String__fini(&msg->database_location);
  // database_version
}

bool
vision_msgs__msg__VisionInfo__are_equal(const vision_msgs__msg__VisionInfo * lhs, const vision_msgs__msg__VisionInfo * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__are_equal(
      &(lhs->header), &(rhs->header)))
  {
    return false;
  }
  // method
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->method), &(rhs->method)))
  {
    return false;
  }
  // database_location
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->database_location), &(rhs->database_location)))
  {
    return false;
  }
  // database_version
  if (lhs->database_version != rhs->database_version) {
    return false;
  }
  return true;
}

bool
vision_msgs__msg__VisionInfo__copy(
  const vision_msgs__msg__VisionInfo * input,
  vision_msgs__msg__VisionInfo * output)
{
  if (!input || !output) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__copy(
      &(input->header), &(output->header)))
  {
    return false;
  }
  // method
  if (!rosidl_runtime_c__String__copy(
      &(input->method), &(output->method)))
  {
    return false;
  }
  // database_location
  if (!rosidl_runtime_c__String__copy(
      &(input->database_location), &(output->database_location)))
  {
    return false;
  }
  // database_version
  output->database_version = input->database_version;
  return true;
}

vision_msgs__msg__VisionInfo *
vision_msgs__msg__VisionInfo__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  vision_msgs__msg__VisionInfo * msg = (vision_msgs__msg__VisionInfo *)allocator.allocate(sizeof(vision_msgs__msg__VisionInfo), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(vision_msgs__msg__VisionInfo));
  bool success = vision_msgs__msg__VisionInfo__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
vision_msgs__msg__VisionInfo__destroy(vision_msgs__msg__VisionInfo * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    vision_msgs__msg__VisionInfo__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
vision_msgs__msg__VisionInfo__Sequence__init(vision_msgs__msg__VisionInfo__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  vision_msgs__msg__VisionInfo * data = NULL;

  if (size) {
    data = (vision_msgs__msg__VisionInfo *)allocator.zero_allocate(size, sizeof(vision_msgs__msg__VisionInfo), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = vision_msgs__msg__VisionInfo__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        vision_msgs__msg__VisionInfo__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
vision_msgs__msg__VisionInfo__Sequence__fini(vision_msgs__msg__VisionInfo__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      vision_msgs__msg__VisionInfo__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

vision_msgs__msg__VisionInfo__Sequence *
vision_msgs__msg__VisionInfo__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  vision_msgs__msg__VisionInfo__Sequence * array = (vision_msgs__msg__VisionInfo__Sequence *)allocator.allocate(sizeof(vision_msgs__msg__VisionInfo__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = vision_msgs__msg__VisionInfo__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
vision_msgs__msg__VisionInfo__Sequence__destroy(vision_msgs__msg__VisionInfo__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    vision_msgs__msg__VisionInfo__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
vision_msgs__msg__VisionInfo__Sequence__are_equal(const vision_msgs__msg__VisionInfo__Sequence * lhs, const vision_msgs__msg__VisionInfo__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!vision_msgs__msg__VisionInfo__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
vision_msgs__msg__VisionInfo__Sequence__copy(
  const vision_msgs__msg__VisionInfo__Sequence * input,
  vision_msgs__msg__VisionInfo__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(vision_msgs__msg__VisionInfo);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    vision_msgs__msg__VisionInfo * data =
      (vision_msgs__msg__VisionInfo *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vision_msgs__msg__VisionInfo__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          vision_msgs__msg__VisionInfo__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!vision_msgs__msg__VisionInfo__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
