...
vision_msgs__msg__LabelInfo__create(void)
{
  ...
  vision_msgs__msg__LabelInfo__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    vision_msgs__msg__LabelInfo * data =
      (vision_msgs__msg__LabelInfo *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vision_msgs__msg__LabelInfo__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          vision_msgs__msg__LabelInfo__fini(&output->data[i]);
        }
        return false;
      }
    }
